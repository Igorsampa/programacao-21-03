...
```
Algoritmo "Sistema de Votos"
VARIÁVEL IDADE = DIGITE(IDADE)
SE (IDADE < 16) 
    ENTÃO (IMPRIMA("NÃO PODE VOTAR!))
SE NÃO SE ( IDADE >= 16 E < 18) 
    ENTÃO (IMPRIMA("VOTO FACULTATIVO!))
SE NÃO (IMPRIMA(VOTO OBRIGATÓRIO))
...
Classe Circulo herda de FormaGeometrica
    Caracteristicas(cor, raio) {
        Metodo Construtor(cor)
        Atributos:
        raio
    }
    Area do circulo() {
        Imprima(A área de um círculo é o seu raio ao quadrado vezes pi)
    }

    Variavel circulo = novo Circulo(Amarelo, 10)

    Imprima(Essa figura tem área de `$circulo`)

Classe Retangulo herda de FormaGeometrica
    Caracteristicas(cor, altura, lagura) {
        Metodo Construtor(cor)
        Atributos:
        altura,
        largura
    ...
    Area do retangulo() {
        imprima(A área de um retangulo é a altura vezes lagura)
    }

    Variavel retangulo = novo Retangulo(Azul, 5, 10)

     Imprima(Essa figura tem área de `$retangulo`)
...
``RESPOSTA``
```
Variável velocidadeInicial = 0
Variável distancia
Variável aceleracao = 0
Variável tempo = 0
Variável velocidade 
 
ENQUANTO (velocidade < 3000 && tempo < 10 minutos) FAÇA {
    tempo += 1 segundo
    velocidade = velocidadeInicial + aceleracao * tempo
    distancia = velocidadeInicial * tempo + (1/2) * aceleracao * tempo^2
}
